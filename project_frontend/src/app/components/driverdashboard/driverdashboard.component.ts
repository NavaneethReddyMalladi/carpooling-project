...
export class DriverDashboardComponent implements OnInit, OnDestroy {
  ...
  loadDriverDetails(driverId: string, token: string) {
    this.http.get<any>(`http://127.0.0.1:42099/users/${driverId}`, {
      ...
      next: (driver) => {
        ...
        console.log('Driver details loaded:', this.driverDetails); // Debug log
        
        // Load all data after driver details are loaded
        this.loadDriverRides();
        this.loadDashboardStats();
        this.loadRideRequests(); // Load requests regardless of online status
  ...
  loadDashboardStats() {
    ...
    if (!token || !this.driverDetails.driver_id) {
      return;          // prevents the “…/driver//stats” request
    }
...
loadRideRequests() {
  const token = localStorage.getItem('token');
  if (!token) {
    console.error('No token found for ride requests');
    return;
  }

  if (!this.driverDetails.driver_id) {
    console.error('Driver ID not available for ride requests');
    return;
  }

  console.log('Loading ride requests for driver:', this.driverDetails.driver_id); // Debug log

  const endpoint = `http://127.0.0.1:42099/ride-requests/driver/${this.driverDetails.driver_id}`;
  
  this.http.get<any>(endpoint, {
    headers: { Authorization: `Bearer ${token}` }
  }).subscribe({
    next: (response) => {
      console.log('Ride requests response:', response); // Debug log
    ...
    error: (err) => {
      ...
      this.error = `Failed to load ride requests: ${err.message || 'Unknown error'}`;
...
// Start polling for ride requests
  startRequestPolling() {
    ...
    this.requestPollingInterval = setInterval(() => {
      console.log('Polling for ride requests...'); // Debug log
  ...
  stopRequestPolling() {
    if (this.requestPollingInterval) {
      console.log('Stopping request polling...'); // Debug log
  ...
  setActiveTab(tab: string) {
    ...
    this.error = '';
    if (tab === 'requests') {
      this.loadRideRequests();   // always fresh data
    }
